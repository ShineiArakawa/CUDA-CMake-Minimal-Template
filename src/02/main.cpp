...
static const char *WIN_TITLE =
    "OpenGL Course"; // ウィンドウのタイトル / Window title
...
void initializeGL() {
  ...
  unsigned char *bytes = stbi_load(TEX_FILE.c_str(), &texWidth, &texHeight,
  ...
  gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA8, texWidth, texHeight, GL_RGBA,
  ...
  // ロードした画素情報の破棄
  // (CPU側のデータを破棄するだけなのでGPU上のテクスチャには影響しない) Free
  // image data (it's on CPU side, so it does not affect texture on GPU)
...
void paintGL() {
  ...
  glBindTexture(GL_TEXTURE_2D,
                textureId); // テクスチャの有効化 / Enable texture mapping
  ...
  glEnd();
  glBindTexture(GL_TEXTURE_2D,
                0); // テクスチャの無効化 / Disable texture mapping
}
